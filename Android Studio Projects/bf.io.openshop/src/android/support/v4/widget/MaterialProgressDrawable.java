...
class MaterialProgressDrawable extends Drawable implements Animatable
{
    ...
    private void setSizeParameters(final double n, final double n2, final double n3, final double n4, final float n5, final float n6) {
        ...
        this.mWidth = n * density;
        this.mHeight = n2 * density;
        mRing.setStrokeWidth(density * (float)n4);
        mRing.setCenterRadius(n3 * density);
    ...
    private void setupAnimators() {
        ...
        final Animation mAnimation = new Animation() {
            public void applyTransformation(final float n, final Transformation transformation) {
                ...
                if (n > 0.5f) {
                    mRing.setEndTrim(startingEndTrim + (0.8f - access$100) * MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation((n - 0.5f) / 0.5f));
                }
    ...
    private static class Ring
    {
        ...
        public void draw(final Canvas canvas, final Rect rect) {
            ...
            final float n = 360.0f * (this.mStartTrim + this.mRotation);
            final float n2 = 360.0f * (this.mEndTrim + this.mRotation) - n;
