...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild
{
    ...
    public SwipeRefreshLayout(final Context context, final AttributeSet set) {
        ...
        this.mAnimateToCorrectPosition = new Animation() {
            public void applyTransformation(final float n, final Transformation transformation) {
                ...
                SwipeRefreshLayout.this.setTargetOffsetTopAndBottom(SwipeRefreshLayout.this.mFrom + (int)(n * (n2 - SwipeRefreshLayout.this.mFrom)) - SwipeRefreshLayout.this.mCircleView.getTop(), false);
    ...
    private void moveSpinner(final float n) {
        ...
        final float n3 = Math.abs(n) - this.mTotalDragDistance;
        ...
        final int n5 = this.mOriginalOffsetTop + (int)(2.0f * (mSpinnerFinalOffset * n4) + mSpinnerFinalOffset * min);
        ...
        this.mProgress.setProgressRotation(0.5f * (-0.25f + 0.4f * n2 + 2.0f * n4));
        this.setTargetOffsetTopAndBottom(n5 - this.mCurrentTargetOffsetTop, true);
    }
    ...
    private Animation startAlphaAnimation(final int n, final int n2) {
        ...
        final Animation animation = new Animation() {
            public void applyTransformation(final float n, final Transformation transformation) {
                SwipeRefreshLayout.this.mProgress.setAlpha((int)(n + n * (n2 - n)));
            }
        };
    ...
    private void startScaleDownReturnToStartAnimation(final int mFrom, final Animation$AnimationListener animationListener) {
        ...
        (this.mScaleDownToStartAnimation = new Animation() {
            public void applyTransformation(final float n, final Transformation transformation) {
                SwipeRefreshLayout.this.setAnimationProgress(SwipeRefreshLayout.this.mStartingScale + n * -SwipeRefreshLayout.this.mStartingScale);
                SwipeRefreshLayout.this.moveToStart(n);
            }
        }).setDuration(150L);
        if (animationListener != null) {
    ...
    public void onNestedScroll(final View view, final int n, final int n2, final int n3, final int n4) {
        ...
        final int n5 = n4 + this.mParentOffsetInWindow[1];
        if (n5 < 0) {
        ...
        if (this.isEnabled() && !this.mReturningToStart && !this.canChildScrollUp() && !this.mNestedScrollInProgress) {
            switch (actionMasked) {
                ...
                case 1: {
                    ...
                    final float n2 = 0.5f * (MotionEventCompat.getY(motionEvent, pointerIndex2) - this.mInitialMotionY);
                    this.mIsBeingDragged = false;
