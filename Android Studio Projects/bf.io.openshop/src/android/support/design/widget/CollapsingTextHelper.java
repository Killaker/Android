...
final class CollapsingTextHelper
{
    ...
    private boolean calculateIsRtl(final CharSequence charSequence) {
        int n = 1;
        if (ViewCompat.getLayoutDirection(this.mView) != n) {
            n = 0;
        }
    ...
    private void calculateUsingTextSize(final float n) {
        ...
        if (this.mText != null) {
            ...
            if (isClose(n, this.mCollapsedTextSize)) {
                n2 = this.mCollapsedBounds.width();
                ...
                n2 = this.mExpandedBounds.width();
                mCurrentTextSize = this.mExpandedTextSize;
                final Typeface mCurrentTypeface2 = this.mCurrentTypeface;
            ...
            if (n2 > 0.0f) {
                b = ((this.mCurrentTextSize != mCurrentTextSize || this.mBoundsChanged || b) && linearText);
            ...
            if (this.mTextToDraw == null || b) {
                ...
                final CharSequence ellipsize = TextUtils.ellipsize(this.mText, this.mTextPaint, n2, TextUtils$TruncateAt.END);
    ...
    public void draw(final Canvas canvas) {
        ...
        if (this.mTextToDraw != null && this.mDrawTitle) {
            ...
            if (b) {
                ...
                final float n2 = this.mTextureDescent * this.mScale;
            }
            else {
                n = this.mTextPaint.ascent() * this.mScale;
                final float n3 = this.mTextPaint.descent() * this.mScale;
            }
            if (b) {
                mCurrentDrawY += n;
            }
            if (this.mScale != 1.0f) {
                canvas.scale(this.mScale, this.mScale, mCurrentDrawX, mCurrentDrawY);
            }
            if (b) {
                canvas.drawBitmap(this.mExpandedTitleTexture, mCurrentDrawX, mCurrentDrawY, this.mTexturePaint);
            }
            else {
                canvas.drawText(this.mTextToDraw, 0, this.mTextToDraw.length(), mCurrentDrawX, mCurrentDrawY, (Paint)this.mTextPaint);
            }
        }
