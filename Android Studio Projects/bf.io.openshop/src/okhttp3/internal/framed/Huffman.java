...
class Huffman
{
    ...
    private void addCode(final int n, final int n2, byte b) {
        ...
        while (b > 8) {
            ...
            if (root.children[n3] == null) {
                root.children[n3] = new Node();
            }
            root = root.children[n3];
        }
    ...
    byte[] decode(final byte[] array) throws IOException {
        ...
        while (i > 0) {
            final Node node2 = node.children[0xFF & n << 8 - i];
    ...
    void encode(final byte[] array, final OutputStream outputStream) throws IOException {
        long n = 0L;
        int i = 0;
        for (int j = 0; j < array.length; ++j) {
            final int n2 = 0xFF & array[j];
            ...
            while (i >= 8) {
                i -= 8;
                outputStream.write((int)(n >> i));
            }
        }
        if (i > 0) {
            outputStream.write((int)(n << 8 - i | 255 >>> i));
        }
    }
    ...
    int encodedLength(final byte[] array) {
        ...
        for (int i = 0; i < array.length; ++i) {
            n += Huffman.CODE_LENGTHS[0xFF & array[i]];
        }
        return (int)(7L + n >> 3);
    }
