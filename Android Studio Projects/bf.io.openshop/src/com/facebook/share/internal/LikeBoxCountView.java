...
public class LikeBoxCountView extends FrameLayout
{
    ...
    private void drawBorder(final Canvas canvas, final float n, final float n2, final float n3, final float n4) {
        ...
        final float n5 = 2.0f * this.borderRadius;
        path.addArc(new RectF(n, n2, n + n5, n2 + n5), -180.0f, 90.0f);
        if (this.caretPosition == LikeBoxCountViewCaretPosition.TOP) {
            path.lineTo(n + (n3 - n - this.caretWidth) / 2.0f, n2);
            path.lineTo(n + (n3 - n) / 2.0f, n2 - this.caretHeight);
            path.lineTo(n + (n3 - n + this.caretWidth) / 2.0f, n2);
        }
        ...
        if (this.caretPosition == LikeBoxCountViewCaretPosition.RIGHT) {
            path.lineTo(n3, n2 + (n4 - n2 - this.caretWidth) / 2.0f);
            path.lineTo(n3 + this.caretHeight, n2 + (n4 - n2) / 2.0f);
            path.lineTo(n3, n2 + (n4 - n2 + this.caretWidth) / 2.0f);
        }
        ...
        if (this.caretPosition == LikeBoxCountViewCaretPosition.BOTTOM) {
            path.lineTo(n + (n3 - n + this.caretWidth) / 2.0f, n4);
            path.lineTo(n + (n3 - n) / 2.0f, n4 + this.caretHeight);
            path.lineTo(n + (n3 - n - this.caretWidth) / 2.0f, n4);
        }
        path.lineTo(n + this.borderRadius, n4);
        path.addArc(new RectF(n, n4 - n5, n + n5, n4), 90.0f, 90.0f);
        if (this.caretPosition == LikeBoxCountViewCaretPosition.LEFT) {
            path.lineTo(n, n2 + (n4 - n2 + this.caretWidth) / 2.0f);
            path.lineTo(n - this.caretHeight, n2 + (n4 - n2) / 2.0f);
            path.lineTo(n, n2 + (n4 - n2 - this.caretWidth) / 2.0f);
        }
        path.lineTo(n, n2 + this.borderRadius);
    ...
    private void setAdditionalTextPadding(final int n, final int n2, final int n3, final int n4) {
        this.likeCountLabel.setPadding(n + this.textPadding, n2 + this.textPadding, n3 + this.textPadding, n4 + this.textPadding);
    }
    ...
    public void setCaretPosition(final LikeBoxCountViewCaretPosition caretPosition) {
