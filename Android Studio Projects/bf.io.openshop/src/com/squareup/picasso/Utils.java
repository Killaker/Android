...
final class Utils
{
    ...
    static long calculateDiskCacheSize(final File file) {
        ...
        try {
            ...
            n = statFs.getBlockCount() * statFs.getBlockSize() / 50L;
    ...
    static boolean isAirplaneModeOn(final Context context) {
        ...
        try {
            final int int1 = Settings$System.getInt(contentResolver, "airplane_mode_on", 0);
            boolean b = false;
            if (int1 != 0) {
                b = true;
            }
    ...
    static boolean isWebPFile(final InputStream inputStream) throws IOException {
        ...
        final int read = inputStream.read(array, 0, 12);
        boolean b = false;
        if (read == 12) {
