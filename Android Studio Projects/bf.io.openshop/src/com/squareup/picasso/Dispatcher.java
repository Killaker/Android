...
class Dispatcher
{
    ...
    void performCancel(final Action action) {
        ...
        final Action action2 = this.failedActions.remove(action.getTarget());
        if (action2 != null && action2.getPicasso().loggingEnabled) {
            Utils.log("Dispatcher", "canceled", action2.getRequest().logId(), "from replaying");
        }
    }
        ...
        if (this.pausedTags.add(o)) {
            ...
            while (iterator.hasNext()) {
                ...
                if (action != null || b) {
                    ...
                    if (b) {
                        for (int i = -1 + actions.size(); i >= 0; --i) {
    ...
    void performRetry(final BitmapHunter bitmapHunter) {
        if (!bitmapHunter.isCancelled()) {
            ...
            final boolean scansNetworkChanges = this.scansNetworkChanges;
